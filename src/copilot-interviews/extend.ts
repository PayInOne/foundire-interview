...
export type CopilotExtendResponse =
  | {
      status: 200
      body: {
        success: true
...
export async function handleExtendCopilotInterview(
  copilotInterviewId: string,
  body: unknown
): Promise<CopilotExtendResponse> {
  try {
    ...
    const { data: copilotInterview, error: fetchError } = await adminSupabase
      ...
      .select('interview_id, company_id, room_status')
    ...
    if (!allowedStatuses.includes(meta.room_status)) {
      return { status: 409, body: { error: `Cannot extend interview in status: ${meta.room_status}` } }
    }
    ...
    console.error('Error in copilot extend:', error)
